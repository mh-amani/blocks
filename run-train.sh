...
CKPT="'/dlabdata1/masani/blocks/logs/training/runs/scan/mixed-[0.04, 0.99]-bart-vqvae/2024-01-17_17-50-19/checkpoints/last.ckpt'"
...
python3 run_train.py +experiment=scan_curriculum.yaml datamodule.dataset_parameters.supervision_ratio=[0.04,0.99] trainer.devices=$DEVICE \
    ...
    callbacks.supervision_scheduler.scheduler_xz.num_warmup_steps=3 callbacks.supervision_scheduler.scheduler_xz.num_training_steps=100 \
    callbacks.supervision_scheduler.scheduler_z.num_warmup_steps=3 callbacks.supervision_scheduler.scheduler_z.num_training_steps=100 \
    +test=True model.optimizer.lr=$LR name=$NAME model.lr_scheduler.monitor="val/loss/zxz" num_epochs=$NUM_EPOCHS logger.wandb.notes="mixed  continueing" logger.wandb.tags=["mixed","continue"]
