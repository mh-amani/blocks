...
CKPT="'/dlabdata1/masani/blocks/logs/training/runs/scan/curriculum-[0.01, 0.99]-bart-vqvae/2024-01-22_10-16-48/checkpoints/model-6156-0.4065.ckpt'"
...
# mixed:
python3 run_train.py +experiment=scan_mixed.yaml datamodule.dataset_parameters.supervision_ratio=[0.01,0.7] trainer.devices=$DEVICE \
    ...
    +test=True model.optimizer.lr=$LR name=$NAME model.lr_scheduler.monitor="val/loss/zxz" num_epochs=$NUM_EPOCHS logger.wandb.notes="zxz and xzx" logger.wandb.tags=["zxz"]
...
python3 run_train.py +experiment=scan_curriculum.yaml datamodule.dataset_parameters.supervision_ratio=[0.01,0.99] trainer.devices=$DEVICE \
    ...
    model.substitute_config.model_params.loss_coeff.supervised_seperated_x=0.0 model.substitute_config.model_params.loss_coeff.supervised_seperated_z=0.0 \
    callbacks.supervision_scheduler.scheduler_xz.hp_init=0.0 callbacks.supervision_scheduler.scheduler_xz.hp_end=0.0 \
